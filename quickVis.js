...
$.fn.quickVis = function (msgBus) {
  ...
  const blankChartOptions = {
    title: {},
    tooltip: {
      trigger: 'axis', // Enable tooltips on axis hover
      axisPointer: {
        type: 'cross', // Show crosshair
        label: {
          backgroundColor: '#6a7985' // Customize tooltip background color
        }
      }
    },
    legend: {
    ...
    try {
      ...
      if (columns && dataset) {
        ...
        const option = {
          ...
          series: series,
          tooltip: { // Add tooltip configuration to the main options
            trigger: 'axis',
            axisPointer: {
              type: 'cross',
              label: {
